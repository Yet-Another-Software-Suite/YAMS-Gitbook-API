...
| Method | Returns | Description |
|--------|---------|-------------|
| `getMechanismPosition()` | `Angle` | Mechanism-space angle after gearing is applied. |
| `getMechanismVelocity()` | `AngularVelocity` | Mechanism-space angular velocity after gearing. |
| `getMechanismAcceleration()` | `AngularAcceleration` | Mechanism-space angular acceleration. |
| `getMeasurementPosition()` | `Distance` | Linear position. Requires `withMechanismCircumference`. |
| `getMeasurementVelocity()` | `LinearVelocity` | Linear velocity. Requires `withMechanismCircumference`. |
| `getMeasurementAcceleration()` | `LinearAcceleration` | Linear acceleration. Requires `withMechanismCircumference`. |
| `getRotorPosition()` | `Angle` | Raw rotor-encoder position scaled to mechanism rotations. |
| `getRotorVelocity()` | `AngularVelocity` | Raw rotor-encoder angular velocity. |
| `getExternalEncoderPosition()` | `Optional<Angle>` | External encoder position if an encoder is attached. |
| `getExternalEncoderVelocity()` | `Optional<AngularVelocity>` | External encoder velocity if an encoder is attached. |
| `getMechanismPositionSetpoint()` | `Optional<Angle>` | Current position setpoint, if set. |
| `getMechanismSetpointVelocity()` | `Optional<AngularVelocity>` | Current velocity setpoint, if set. |
