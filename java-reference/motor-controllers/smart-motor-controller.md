...
| `setVelocity(AngularVelocity velocity, Force feedforwardForce)` | `void` | Commands the mechanism to the given angular velocity, with an additional feedforward `Force` applied on top (e.g. from a PathPlanner set-point generator). Requires `withGearing`/`withMechanismCircumference` to be set. Pass `null` to fall back to plain velocity control. |
| `setVelocity(LinearVelocity velocity, Force feedforwardForce)` | `void` | Linear-velocity counterpart of the above. |

{% hint style="info" %}
`Force` here means the drive-wheel feedforward force from a set-point generator such as PathPlanner's `DriveFeedforwards.linearForces()`, not a torque or a raw voltage. `SmartMotorControllerConfig.convertToVoltage(...)`/`convertToCurrent(...)` convert it into the correct motor-level feedforward using the configured gearing and mechanism circumference (wheel radius) — see [SmartMotorControllerConfig](smart-motor-controller-config.md#feedforward-conversion). A `null` force is treated identically to calling the no-force overload: no feedforward is applied, and `getSetpointFeedforwardForce()` reads back empty.
{% endhint %}

## Measurement

<table><thead><tr><th>Method</th><th>Returns</th><th width="250">Description</th></tr></thead><tbody><tr><td><code>getMechanismPosition()</code></td><td><code>Angle</code></td><td>Mechanism-space angle after gearing is applied.</td></tr><tr><td><code>getMechanismVelocity()</code></td><td><code>AngularVelocity</code></td><td>Mechanism-space angular velocity after gearing.</td></tr><tr><td><code>getMechanismAcceleration()</code></td><td><code>AngularAcceleration</code></td><td>Mechanism-space angular acceleration.</td></tr><tr><td><code>getMeasurementPosition()</code></td><td><code>Distance</code></td><td>Linear position. Requires <code>withMechanismCircumference</code>.</td></tr><tr><td><code>getMeasurementVelocity()</code></td><td><code>LinearVelocity</code></td><td>Linear velocity. Requires <code>withMechanismCircumference</code>.</td></tr><tr><td><code>getMeasurementAcceleration()</code></td><td><code>LinearAcceleration</code></td><td>Linear acceleration. Requires <code>withMechanismCircumference</code>.</td></tr><tr><td><code>getRotorPosition()</code></td><td><code>Angle</code></td><td>Raw rotor-encoder position scaled to mechanism rotations.</td></tr><tr><td><code>getRotorVelocity()</code></td><td><code>AngularVelocity</code></td><td>Raw rotor-encoder angular velocity.</td></tr><tr><td><code>getExternalEncoderPosition()</code></td><td><code>Optional&#x3C;Angle></code></td><td>External encoder position if an encoder is attached.</td></tr><tr><td><code>getExternalEncoderVelocity()</code></td><td><code>Optional&#x3C;AngularVelocity></code></td><td>External encoder velocity if an encoder is attached.</td></tr><tr><td><code>getMechanismPositionSetpoint()</code></td><td><code>Optional&#x3C;Angle></code></td><td>Current position setpoint, if set.</td></tr><tr><td><code>getMechanismSetpointVelocity()</code></td><td><code>Optional&#x3C;AngularVelocity></code></td><td>Current velocity setpoint, if set.</td></tr><tr><td><code>getSetpointFeedforwardForce()</code></td><td><code>Optional&#x3C;Force></code></td><td>Feedforward force last supplied to <code>setVelocity(velocity, Force)</code>. Empty if the no-force overload (or a <code>null</code> force) was used most recently.</td></tr></tbody></table>
